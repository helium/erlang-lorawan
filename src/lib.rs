...
pub fn create_mac(fcnt: u32) -> &'static [u16] {
	...
	let array_u8: [u8; 32] = tag_bytes.as_slice().try_into().expect("Wrong length");
...
pub fn mutate_state(fcnt: u32, mic: &[u16], state: array2d::Array2D<u16>) -> array2d::Array2D<u16> {
...
pub fn test_mic() {
	...
	let state1 = mutate_state(fcnt, mac0, state0);
	...
	let state2 = mutate_state(fcnt, mac1, state1);
	//let _mic1 = generate_tag(fcnt, state2);
	fcnt = fcnt + 1;
	let mac2 = create_mac(fcnt);
	let state3 = mutate_state(fcnt, mac2, state2);
	//let _mic2 = generate_tag(fcnt, state3);
	fcnt = fcnt + 1;
	let mac3 = create_mac(fcnt);
	let state4 = mutate_state(fcnt, mac3, state3);
